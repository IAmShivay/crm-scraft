    ...
    case "POST": {
      if (action === "createStatus") {
        ...
        try {
          ...
          // Log the status creation activity
          if (data) {
            try {
              await ActivityLogger.logStatusCreated(
                workspaceId,
                user.id,
                user.email || '',
                {
              ...
              console.error('Failed to log status creation activity:', logError);
        ...
        try {
          ...
          try {
            await ActivityLogger.logActivity({
              workspace_id: statusData.work_id.toString(),
              user_id: user.id,
              member_email: user.email || '',
              member_name: user.user_metadata?.name || user.user_metadata?.firstName,
              activity_type: 'status_updated' as any,
              activity_description: `Status "${name}" updated by ${user.email}`,
              metadata: {
                status_id: id,
                status_name: name,
            ...
            console.error('Failed to log status update activity:', logError);
          }
        ...
        try {
          ...
          try {
            await ActivityLogger.logActivity({
              ...
              member_email: user.email || '',
              member_name: user.user_metadata?.name || user.user_metadata?.firstName,
              activity_type: 'status_deleted' as any,
              activity_description: `Status "${statusData.name}" deleted by ${user.email}`,
              metadata: {
                status_id: Array.isArray(id) ? id[0] : id,
            ...
            console.error('Failed to log status deletion activity:', logError);
          }
